  ...
  return (
    <button
      ...
      className={cx(
        ...
        v === "primary" && "bg-slate-900 text-white hover:bg-slate-800",
        v === "secondary" && "bg-slate-100 text-slate-900 hover:bg-slate-200",
        v === "danger" && "bg-rose-600 text-white hover:bg-rose-700",
        className
      )}
    />
  );
...
export function Label(props: React.HTMLAttributes<HTMLDivElement> & { className?: string }) {
  const { className, ...rest } = props;
...
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  ...
  return (
    <select
      {...rest}
      className={cx(
        "w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:border-slate-400 focus:outline-none",
        className
      )}
    />
  );
...
export function Th(props: ThProps) {
  ...
  return (
    <th
      {...rest}
      className={cx(
        "whitespace-nowrap border-b border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600",
        className
      )}
    />
  );
